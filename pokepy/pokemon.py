...
from pokepy.utils import (average, frac, offset_hp_keys, push,
...
class Pokemon:
    ...
    """
    # クラス変数として追加（pokemon.py の上部、クラス定義の直後に追加）
    current_pool_name = None
    permitted_pool = set()
    permitted_items = set()  # ★使用可能アイテムの制限リスト
    ...
    def __init__(self, name: str = "ピカチュウ", use_template: bool = True):
        """{name}のポケモンを生成する。"""
        # 1. 内部変数を先にすべて初期化（これで AttributeError を防ぐ）
        self.__name = name
        ...
        # 2. setter を呼んでデータを正しくロード
        self.name = name
        self.org_ability = Pokemon.zukan[name]["ability"][0] if name in Pokemon.zukan else ""
        self.ability = self.org_ability
    ...
    def reset_game(self):
        ...
        self.status_turns = 0  # ◀︎【追加】確定解凍/起床用のターン管理変数
    ...
    def come_back(self):
        ...
        self.status_turns = 0  # ◀︎【追加】控えに戻った際にターン数をリセット
    ...
    def update_status(self, keep_damage=False):
        ...
        # HP ＝ 種族値 + 75 + SP
        self.__status[0] = self.base[0] + 75 + self.__sp[0]

        # その他 ＝ floor((種族値 + 20 + SP) * 性格補正)
        for i in range(1, 6):
            base_calc = self.base[i] + 20 + self.__sp[i]
            self.__status[i] = math.floor(base_calc * nc[i])
    ...
    def apply_random_template(self):
        """統計データ(home)の採用率に基づいて、性格・特性・持ち物・テラスタイプ・技を確率的に選択します。
        弾かれた技がある場合は、learnset.jsonに定義されている他の技から自動補完し、必ず4つ習得させます。
        """
        if self.__name in Pokemon.home:
            h = Pokemon.home[self.__name]
            allowed_moves = Pokemon.learnsets.get(self.__name, [])  # 習得可能リスト

            # 安全に確率選択を行うためのヘルパー
            def safe_choices(population, weights):
                if not population:
                    return None
            ...
            # 1. 性格を採用率の重みに基づいてランダム選択
            self.__nature = random.choice(list(Pokemon.nature_corrections.keys()))

            # 2. 特性を採用率の重みに基づいてランダム選択
            if self.__name in Pokemon.zukan:
                self.org_ability = random.choice(Pokemon.zukan[self.__name]["ability"])
                self.ability = self.org_ability

            # 3. 持ち物を採用率の重みに基づいてランダム選択（現在のレギュレーションに基づいて制限）
            if h.get("item") and h["item"][0]:
                item_pool = list(h["item"][0])
                item_weights = list(h["item"][1])

                def is_item_permitted(p_name: str, item_name: str) -> bool:
                    # メガストーンの場合：対応するポケモン以外の持ち込みは常に禁止
                    if item_name.endswith("ナイト"):
                        stone_base = item_name[:-3]
                        return stone_base in p_name

                    # テンプレート以外の専用アイテム制限
                    if item_name == "でんきだま" and p_name != "ピカチュウ":
                        return False
                    if item_name == "しんかのきせき" and p_name in ["ガブリアス", "ハッサム", "ブリジュラス",
                                                                    "カイリュー"]:
                        return False

                    # ★レギュレーションファイルによる一元管理の適用
                    # reg_mb_top100_items.txt の中身（ホワイトリスト）に定義されているもののみ許可する
                    if hasattr(Pokemon, "mb_items") and Pokemon.mb_items:
                        return item_name in Pokemon.mb_items

                    return True

                filtered_pool = [item for item, weight in zip(item_pool, item_weights) if
                                 is_item_permitted(self.__name, item)]
                filtered_weights = [weight for item, weight in zip(item_pool, item_weights) if
                                    is_item_permitted(self.__name, item)]

                if filtered_pool:
                    self.item = safe_choices(filtered_pool, filtered_weights)
                else:
                    self.item = ""

            # 4. テラスタイプを採用率の重みに基づいてランダム選択
            if h.get("Ttype") and h["Ttype"][0]:
                self.Ttype = safe_choices(h["Ttype"][0], h["Ttype"][1])

            # 5. 技を採用率の重みに基づいて、重複を避けて4つランダム選択
            if h.get("move") and h["move"][0]:
                move_pool = list(h["move"][0])
                move_weights = list(h["move"][1])
                chosen_moves = []
                for _ in range(4):
                    if not move_pool:
                        break
                    m = safe_choices(move_pool, move_weights)
                    chosen_moves.append(m)
                    idx = move_pool.index(m)
                    move_pool.pop(idx)
                    move_weights.pop(idx)

                # いったんセット（覚えない技はmovesセッター内で自動で弾かれます）
                self.moves = chosen_moves

            # ▽ 【追加】弾かれた結果、技が4つ未満になった場合に他の覚える技で自動補完する
            if len(self.moves) < 4 and allowed_moves:
                # すでに覚えている技を除いた、習得可能候補リスト
                available_fillers = [m for m in allowed_moves if m not in self.moves]
                while len(self.moves) < 4 and available_fillers:
                    filler_move = random.choice(available_fillers)
                    self.add_move(filler_move)  # 技を追加（PP標準化なども走ります）
                    available_fillers.remove(filler_move)

            # 6. 努力値（SP）の配分を完全ランダム（微調整やフラット型も含む）に配分する
            # （合計66、各単体32以下を保証しながら確率的に分散させます）
            random_sp = [0] * 6
            available_sp = 66

            while available_sp > 0:
                # まだ上限の 32 に達していないステータスの位置を取得
                valid_indices = [i for i, val in enumerate(random_sp) if val < 32]
                ...
                # その能力に配分可能な「最大値」を計算
                max_alloc = min(32 - random_sp[idx], available_sp)

                # 1 から最大値の間で、ランダムなポイント（チャンク）を割り振る
    ...
    # Setter
    @name.setter  # プロパティデコレータがあるか確認してください
    def name(self, name: str):
        if name not in Pokemon.zukan:
            ...
            # 【追加】name の方でも体重のKeyErrorを防ぐ
            if "weight" in Pokemon.zukan[self.__name]:
                self.__weight = Pokemon.zukan[self.__name]["weight"]
            else:
                self.__weight = 0.0  # 体重データがなければ一旦 0.0 にする
    ...
    def change_form(self, name: str) -> None:
        """指定された名前のフォルムに変更する"""
        self.__name = name
        self.__types = Pokemon.zukan[name]["type"]

        # ▽ ここを追加：新しいフォルム（メガシンカ後）の特性を適用する
        if "ability" in Pokemon.zukan[name] and Pokemon.zukan[name]["ability"]:
            self.org_ability = Pokemon.zukan[name]["ability"][0]
            self.ability = self.org_ability

        # 修正：体重がzukanに登録されていない場合は、既存の体重をキープするか 0.0 にする
        if "weight" in Pokemon.zukan[name]:
            self.__weight = Pokemon.zukan[name]["weight"]
        else:
            # 登録されていない場合は何もしない（変身前の体重をキープ）
            pass

        self.__base = Pokemon.zukan[name]["base"].copy()
    ...
    def status(self, status: list[int]):
        ...
        for i in range(6):
            found = False
            for s_points in range(33):
                if i == 0:
                    v = self.__base[0] + 75 + s_points
                else:
                    v = math.floor((self.__base[i] + 20 + s_points) * nc[i])
                if v == status[i]:
                    self.__sp[i] = s_points
                    self.__status[i] = v
                    found = True
                    break
            if not found:
    ...
    def set_status(self, index: int, value: int) -> bool:
        ...
        for s_points in range(33):
            if index == 0:
                ...
                v = math.floor((self.__base[index] + 20 + s_points) * nc[index])
            if v == value:
                self.__sp[index] = s_points
    ...
    @property
    def sp(self):
        return self.__sp.copy()

    @sp.setter
    def sp(self, value: list[int]):
        if sum(value) > 66:
            raise ValueError(f"SP合計値({sum(value)})は66を超えることはできません。")
        if any(s > 32 for s in value):
            raise ValueError("各ステータスのSP上限は32ポイントです。")
        self.__sp = value.copy()
        self.update_status()

    @property
    def effort(self):
        """互換性維持：外部からeffortを呼ばれたらSPのコピーを返す"""
        return self.__sp.copy()

    @effort.setter
    def effort(self, effort: list[int]):
        """互換性維持：もし大きな努力値データ（例: 252）が来たらSPに安全に自動変換する"""
        if any(val > 32 for val in effort):
            normalized_sp = [min(32, int(val / 8)) for val in effort]
            if sum(normalized_sp) > 66:
                normalized_sp = [0] * 6
            self.__sp = normalized_sp
        else:
            self.__sp = effort.copy()
        self.update_status()

    def set_effort(self, index: int, value: int):
        """互換性維持：特定のステータスのSPを個別に変更する"""
        if value > 32:
            value = min(32, int(value / 8))
        self.__sp[index] = value
        self.update_status()

    def sanitize_to_standard_pp(self, move_name: str, base_pp: int) -> int:
        """基本PPから 8 / 12 / 16 / 20 に標準化する"""
        nerfed_pp_moves = [
            "ままもる", "まもる", "みきり", "ニードルガード", "キングシールド",
            "すなあらし", "ゆきげしき", "くちばしキャノン"
        ]
        if move_name in nerfed_pp_moves:
            return 8
        if move_name == "つじぎり":
            return 20
    ...
    def moves(self, moves: list[str]):
        """チャンピオンズ没収技フィルタとPP標準化を適用して技を登録する"""
        self.__moves, self.pp = [], []

        # 1. 習得リストを取得（一番最初に定義する）
        allowed_moves = Pokemon.learnsets.get(self.__name, [])

        confiscated_moves = {
            "サーフゴー": ["でんじは"],
            "コノヨザル": ["いのちがけ"],
            "メタグロス": ["ヘビーボンバー", "はたきおとす"],
            "オーロンゲ": ["でんじは", "どげざつき"],
            "ズルズキン": ["すてゼリフ"],
            "ブリジュラス": ["ボディプレス"],
            "ガルーラ": ["グロウパンチ"],
            ...
            "ガオガエン": ["はたきおとす", "とんぼがえり"],
            "ロトム": ["すてゼリフ", "パラボラチャージ"],
            "ヒートロトム": ["すてゼリフ", "パラボラチャージ"],
            "ウォッシュロトム": ["すてゼリフ", "パラボラチャージ"],
            "フロストロトム": ["すてゼリフ", "パラボラチャージ"],
            "スピンロトム": ["すてゼリフ", "パラボラチャージ"],
            "カットロトム": ["すてゼリフ", "パラボラチャージ"],
        }
        ...
        for move in moves:
            ...
            if display_name in confiscated_moves and move in confiscated_moves[display_name]:
                continue

            # 3. 習得技フィルター（learnset.jsonにあればチェックする）
            if allowed_moves and move not in allowed_moves:
                warnings.warn(f"{self.__name} は {move} を覚えないため無視します")
                continue

            # 4. 技の登録
            if move not in Pokemon.all_moves:
                warnings.warn(f"{move} is not in Pokemon.all_moves")
            else:
                self.__moves.append(move)
                base_pp = Pokemon.all_moves[move]["pp"] // 1.6
                std_pp = self.sanitize_to_standard_pp(move, int(base_pp))
                self.pp.append(std_pp)
    ...
    @classmethod
    def init(cls, season=None, usage_data_path: str | None = None, pool_name: str | None = None, verbose: bool = True):
        """ライブラリを初期化する"""
        ...
        cls.home = {}
        cls.abilities = []
        cls.ability_category = {}
        cls.items = {}
        cls.item_buff_type = {}
        cls.item_debuff_type = {}
        cls.item_correction = {}
        cls.consumable_items = []
        cls.all_moves = {}
        cls.move_category = {}
        cls.move_value = {}
        cls.move_priority = {}
        ...
        cls.move_effect = {}
        cls.type_id = {}
        cls.type_corrections = []
        cls.nature_corrections = {}
        # -----------------------------

        try:
            with open("data/learnset.json", encoding="utf-8") as f:
                Pokemon.learnsets = json.load(f)
        except FileNotFoundError:
            if verbose:
                print("⚠️ 習得技リスト(learnset.json)が見つかりません。制限なしで起動します。")
        ...
        if season is None:
            ...
            # ▽ インデントを1段階左にずらし、if season is None: の外に出しました
            # プール制限リスト（レギュレーション）の読み込み
        cls.current_pool_name = pool_name
        cls.permitted_pool = set()
        cls.permitted_items = set()

        if pool_name:
            pool_path = f"data/pool/{pool_name}.txt"
            item_pool_path = f"data/pool/{pool_name}_items.txt"  # ★アイテム用のプール

            # 1. ポケモンプールのロード
            try:
                with open(pool_path, encoding="utf-8") as f:
                    for line in f:
                        p_name = to_hankaku(line.strip())
                        if p_name:
                            cls.permitted_pool.add(p_name)
                if verbose:
                    print(
                        f"✅ [POOL SYSTEM] レギュレーション '{pool_name}' ポケモンプール適用完了: {len(cls.permitted_pool)}種")
            except FileNotFoundError:
                if verbose:
                    print(f"⚠️ [POOL SYSTEM] ポケモンプール '{pool_path}' が見つかりません。")

            # 2. アイテムプールのロード（存在する場合のみ適用）
            try:
                with open(item_pool_path, encoding="utf-8") as f:
                    for line in f:
                        i_name = to_hankaku(line.strip())
                        if i_name:
                            cls.item_debuff_type.get(i_name)  # ダミー処理、ロード確認
                            cls.permitted_items.add(i_name) if hasattr(cls, 'permitted_items') else None
                            # クラス変数に保存
                            if not hasattr(Pokemon, "mb_items"):
                                Pokemon.mb_items = set()
                            Pokemon.mb_items.add(i_name)
                if verbose:
                    print(
                        f"✅ [POOL SYSTEM] レギュレーション '{pool_name}' アイテムプール適用完了: {len(Pokemon.mb_items)}種")
            except FileNotFoundError:
                if verbose:
                    print(
                        f"ℹ️ [POOL SYSTEM] アイテムプール '{item_pool_path}' は定義されていないため、全アイテムを許可します。")
        ...
        with open("data/zukan.txt", encoding="utf-8") as fin:
            ...
            for line in fin:
                ...
                cls.zukan[name] = {}
                cls.zukan[name]["type"] = [s for s in data[2:4] if s != "-"]
                cls.zukan[name]["ability"] = [s for s in data[4:8] if s != "-"]
                cls.zukan[name]["base"] = list(map(int, data[8:14]))

                for s in cls.zukan[name]["ability"]:
                    cls.abilities.append(s)
                ...
                if "ロトム" in name:
                    ...
                    display_name = display_name.replace("パルデア", "").replace("ヒスイ", "").replace("ガラル",
                                                                                                      "").replace(
                        "アローラ", "").replace("ホワイト", "").replace("ブラック", "")
                cls.zukan[name]["display_name"] = display_name

                if display_name not in cls.zukan_name:
                    cls.zukan_name[display_name] = [name]
                elif name not in cls.zukan_name[display_name]:
                    cls.zukan_name[display_name].append(name)
                    # フォルム違いの差分を記録
                    for key in ["type", "ability"]:
                        if cls.zukan[cls.zukan_name[display_name][0]][key] != cls.zukan[name][key]:
                            cls.form_diff[display_name] = key
                            break
        ...
        with open("data/foreign_name.txt", encoding="utf-8") as fin:
            ...
            with open("data/weight.txt", encoding="utf-8") as fin:
                ...
                for line in fin:
                    data = line.split()
                    name = to_hankaku(data[0])
                    if name in cls.zukan:  # ◀︎【追加】図鑑に存在する（プール内である）場合のみ代入する
                        cls.zukan[name]["weight"] = float(data[1])
    ...
    def set_ailment(
        ...
        if not ailment:
            if p1.ailment:
                ...
                p1.ailment = ""
                # こおり状態が解除された場合、カウンターをリセット
                if hasattr(p1, 'freeze_count'):
                    p1.freeze_count = 0
                return True
            else:
                return False
        ...
        for player in self.action_order:
            ...
            if not any(self.breakpoint):
                ...
                # こおり判定
                    ...
                    # ターン経過による確定解凍（3回目の行動時に確定解凍）
                    if not hasattr(self.pokemon[player], 'freeze_count') or self.pokemon[
                        player].freeze_count == 0:
                        self.pokemon[player].freeze_count = 3

                    self.pokemon[player].freeze_count -= 1

                    if (
                            move in Pokemon.move_category["unfreeze"]
                            or self.pokemon[player].freeze_count <= 0
                            or self._random.random() < 0.2
                    ):
                        self.set_ailment(player, "")
                        self.pokemon[player].freeze_count = 0
                    else:
            ...
            # わざが無効なら中断
                ...
                # まもる判定（M-B仕様：ふかしのこぶしが守るを貫通した際、ダメージが25%に低下）
                is_penetrated_by_fufuki = False
                if (
                        self.protect
                    ...
                    if self.was_valid[player2]:
                        # 接触時の追加効果判定（貫通した場合でも、トーチカや火炎の守りのペナルティは受ける）
                        ...
                        # ふかしのこぶしで貫通していない場合のみ、ここで行動を終了（continue）する
                        if not is_penetrated_by_fufuki:
                            continue
                ...
                for i in range(n_hit):
                    ...
                    if Pokemon.all_moves[move]["class"] in ["phy", "spe"]:
                        ...
                        if Pokemon.all_moves[move]["power"] > 0:
                            ...
                            # 【追加】ふかしのこぶし（守る貫通）時：ダメージを25%に減衰
                            if is_penetrated_by_fufuki:
                                self.damage[player] = max(1, int(self.damage[player] * 0.25))
        ...
        if not any(self.breakpoint):
            ...
            for player in self.speed_order:
                ...
                if p.condition["shiozuke"] and p.hp:
                    ...
                    if self.add_hp(player, -int(p.status[0] / 16 * r)):
