...
from pokepy.pokemon import Pokemon, Damage  # localからロード
from pokepy.utils import (frac, offset_hp_keys, push,
                          round_half_down, round_half_up,
                          zero_ratio)  # utilsからロード

# battle.py の上部に関数群のインポートを追加
from pokepy.utils import *
...
class Battle:
    ...
    def __init__(self, seed: int = None, tera_enabled: bool = True):  # 引数追加
        self.tera_enabled = tera_enabled  # 追加
    ...
    def can_terastal(self, player: int) -> bool:
        if not self.tera_enabled: # 追加
            return False          # 禁止なら即座にFalseを返す
    ...
    def reset_game(self):
        ...
        # ▽ ここに追加：1試合1回限りのメガシンカ権利フラグ
        self.mega_enabled = [True, True]
    ...
    def get_mega_name(self, p: Pokemon) -> str:
        """ポケモンが対応するメガストーンを保持している場合に、メガシンカ後の名称を返します。"""
        if not p or not p.item:
            return ""

        # 分岐するポケモン（リザードン、ミュウツーなど）の個別処理
        if p.name == "リザードン":
            if p.item == "リザードナイトX": return "メガリザードンX"
            if p.item == "リザードナイトY": return "メガリザードンY"
        elif p.name == "ミュウツー":
            if p.item == "ミュウツナイトX": return "メガミュウツーX"
            if p.item == "ミュウツナイトY": return "メガミュウツーY"

        # 通常のメガシンカ（例: ガブリアス ＋ ガブリアスナイト ＝ メガガブリアス）
        if p.item.endswith("ナイト"):
            stone_base = p.item[:-3]  # 「ナイト」を除いた文字列（例：「ガブリアス」）
            if stone_base in p.name:
                mega_name = f"メガ{p.name}"
                if mega_name in Pokemon.zukan:
                    return mega_name
        return ""

    def can_mega_evolve(self, player: int) -> bool:
        """対象プレイヤーの場に出ているポケモンがメガシンカ可能か判定します。"""
        if not self.mega_enabled[player]:
            return False
        p = self.pokemon[player]
        if not p or p.hp == 0:
            return False
        if p.name.startswith("メガ"):
            return False
        return bool(self.get_mega_name(p))
        ...
        for i in range(max_loop):
            ...
            for move, damage_dict in zip(moves, damage_dict_list):
                if move != "END":
                    ...
                    # しおづけダメージ（M-B仕様：1/16、水・鋼は1/8に半減）
    ...
    def available_commands(self, player: int, phase: str = "battle"):
        ...
        if phase == "battle":
            ...
            # ▽ ここに追加：メガシンカして技を選択（コマンド 60〜69）
            if self.can_mega_evolve(player):
                # 通常の有効な技コマンドに +60 オフセットを加えて追加
                commands += [cmd + 60 for cmd in commands if cmd in range(10)]
    ...
    def proceed(
        ...
        if not any(self.breakpoint):
            ...
            for player, p in enumerate(self.pokemon):
                ...
                # メガシンカコマンド（60〜69）の処理
                    ...
                    mega_name = self.get_mega_name(p)
                    if mega_name:
                        self.log[player].append(f"{p.name}が{p.item}に反応してメガシンカ！")

                        # 1. 変化する「前」の元の名前（p.name = "ガブリアス"）で観測データを確保する
                        p_obs = Pokemon.find(self.observed[player], name=p.name)

                        # 2. 本体のメガシンカを実行（名前が "メガガブリアス" になり、ステータス・新特性が適用されます）
                        p.change_form(mega_name)
                        self.mega_enabled[player] = False  # 使用権を消費

                        # ▽ ここを追加：メガシンカによって得た新特性（ひでり等）を即時割り込み発動させる
                        self.release_ability(player)

                        # 3. 確保しておいた観測データも、同時にメガシンカさせる
                        if p_obs:
                            p_obs.change_form(mega_name)

                    # 4. メガシンカした上で、選択した技（0〜9番目）を設定する
                    self.move[player] = p.moves[self.command[player] % 10]
        ...
        for player in self.action_order:
            ...
            if not any(self.breakpoint):
                ...
                # こおり判定（M-B仕様：毎ターン25%で解凍、かつ3回目行動時に確定解凍）
                elif self.pokemon[player].ailment == "FLZ":
                    self.pokemon[player].status_turns += 1

                    # テストとの互換性を確保するため、freeze_countの減算処理も同期させる
                    if hasattr(self.pokemon[player], 'freeze_count'):
                        self.pokemon[player].freeze_count -= 1
                    else:
                        self.pokemon[player].freeze_count = 2

                    if (
                            move in Pokemon.move_category["unfreeze"]
                            or self.pokemon[player].status_turns >= 3
                            or self.pokemon[player].freeze_count <= 0
                            or self._random.random() < 0.25
                    ):
                        self.set_ailment(player, "")
                ...
                if (
                    ...
                    # 技の観測
                    p_obs = Pokemon.find(
                        self.observed[player], display_name=self.pokemon[player].display_name
                    )
                ...
                # 強制反動技（M-B仕様：はめつのひかりの反動を1/2に増加）
                if self.was_valid[player]:
                    rebound_ratio = 0.0
                    if move == "はめつのひかり":
                        rebound_ratio = 0.5
                    elif move in Pokemon.move_value["force_rebound"]:
                        rebound_ratio = Pokemon.move_value["force_rebound"][move]

                    if rebound_ratio > 0:
                        if self.add_hp(
                                player,
                                -round_half_up(self.pokemon[player].status[0] * rebound_ratio)
                        ):
                    ...
                    if Pokemon.all_moves[move]["class"] in ["phy", "spe"]:
                        ...
                        if self.damage[player] == 0:
                            ...
                            match move:
                                ...
                                case "ゴールドラッシュ":
                                    # 【追加】ゴールドラッシュ使用後、特攻を2段階低下にナーフ
                                    self.add_rank(player, 3, -2)
                                case "プラズマフィスト":
                                    pass
